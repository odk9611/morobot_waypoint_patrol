...
void Patrol_start()
{
    ...
    m_patrol_location_cnt = 2;

    location_name[0] = "POINT1";
    location_name[1] = "POINT2";
    ...
    for(int i=0; i<m_patrol_location_cnt; i++)
    {
        arr_patrol_location[i] = location_name[i];
        ROS_INFO("# arr_patrol_location[%d]:%s ", i, arr_patrol_location[i].c_str());
    }
}
...
void joyCallback(const sensor_msgs::Joy::ConstPtr& joy)
{
    ...
    else if(joy->buttons[4] == 1) //Patrol start
    {
        Patrol_start();
    }
    else if(joy->buttons[0] == 1) //Patrol finish
    {
        Patrol_finish();
    }
